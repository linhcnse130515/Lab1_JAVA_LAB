    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (quantity > 0) {
                ...
                CakeDTO dto = new CakeDTO(cakeId, name, price, quantity);
                Cart cart = (Cart) session.getAttribute("CART");
                CakeDAO dao = new CakeDAO();
                if (dao.getQuantity(cakeId) >= quantity) {
                    cart.update(dto);
                    session.setAttribute("CART", cart);
                    total += (quantity - oldQuantity) * dto.getPrice();
                } else {
            ...
            log("Exception at UpdateCartController  " + ex.getMessage());
        } finally {
